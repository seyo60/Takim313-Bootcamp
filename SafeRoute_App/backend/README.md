...
Bu komut, `h3_heatmap`, `reports`, `report_events`, `user_profiles` ve `etl_runs`
tablolarını oluşturur. Güncel heatmap kanalları `risk_crime`, `risk_lighting`,
`risk_live` ve `total_risk` alanlarıdır. Eski `risk_historical` ve `risk_social`
alanları head migration'da bulunmaz.
...
Her H3 hücresinin nihai risk skoru (`total_risk`), üç kanonik kanalın ağırlıklı toplamıdır:

```
total_risk = (risk_crime × 0.65) + (risk_lighting × 0.20) + (risk_live × 0.15)
```

- **risk_crime**: Chicago açık suç verisi ETL'inden gelir.
- **risk_lighting**: Chicago 311 aydınlatma taleplerinden, durum ve zaman azalmasıyla üretilir.
- **risk_live**: Doğrulanan yakın dönem kullanıcı ihbarlarından bounded accumulation ile birikir.
...
├── llm_service.py             # DeepSeek structured-output + retry/circuit/fallback
