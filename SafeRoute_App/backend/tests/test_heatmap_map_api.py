...
class FakeH3Point:
    def __init__(self, h3_index, lat, lng, crime, lighting, live, total):
...
def override_db_dependency():
    ...
    async def fake_get_all_heatmap_points(db):
        return fake_points
...
def test_get_heatmap_map_no_data_cells():
    ...
    assert res_inc.json()["metadata"]["feature_count"] == 3

    res_exc = client.get("/api/v1/heatmap/map?include_no_data=false")
    assert res_exc.status_code == 200
    assert res_exc.json()["metadata"]["feature_count"] == 2  # cell with total_risk=0.0 excluded
