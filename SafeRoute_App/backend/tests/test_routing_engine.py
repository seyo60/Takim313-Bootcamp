...
def test_both_engines_choose_reasonable_green_detour_over_red_shortcut():
    """Eski doğrusal maliyetin seçtiği kırmızı kestirme iki motorda da elenir."""
    graph = nx.MultiDiGraph()
    graph.add_node("S", x=-87.6400, y=41.8700)
    graph.add_node("E", x=-87.6390, y=41.8700)
    graph.add_node("A", x=-87.6395, y=41.8705)

    red_cost = risk_adjusted_length(100.0, 0.95)
    green_cost = risk_adjusted_length(130.0, 0.10)
    graph.add_edge(
        "S", "E", 0,
        length=100.0,
        risk_weight=0.95,
        risk_data_available=True,
        risk_adjusted_length=red_cost,
    )
    graph.add_edge(
        "S", "A", 0,
        length=130.0,
        risk_weight=0.10,
        risk_data_available=True,
        risk_adjusted_length=green_cost,
    )
    graph.add_edge(
        "A", "E", 0,
        length=130.0,
    ...
    nx_engine = NetworkXEngine()
    nx_engine.graph = graph
    nx_engine.find_nearest_node = lambda lat, lng: (
        ("S" if lng < -87.6395 else "E"),
        0.0,
    )
    _, nx_distance, _, nx_risk, _ = nx_engine.compute_safe_route(
        41.8700, -87.6400, 41.8700, -87.6390
    )

    compact = CompactCSREngine()
    compact.node_x = np.array([-87.6400, -87.6390, -87.6395], dtype=np.float64)
    compact.node_y = np.array([41.8700, 41.8700, 41.8705], dtype=np.float64)
    compact.edge_src = np.array([0, 0, 2], dtype=np.int32)
    compact.edge_dst = np.array([1, 2, 1], dtype=np.int32)
    compact.edge_length = np.array([100.0, 130.0, 130.0], dtype=np.float64)
    compact.edge_risk = np.zeros(3, dtype=np.float32)
    compact.edge_has_data = np.zeros(3, dtype=np.bool_)
    compact.h3_keys_map = {
        "red": np.array([0], dtype=np.int32),
        "green": np.array([1, 2], dtype=np.int32),
    }
    compact.N = 3
    compact.M = 3
    compact.csr_shortest = _build_deduplicated_csr(
        compact.N, compact.edge_src, compact.edge_dst, compact.edge_length
    )
    compact.csr_shortest_b = _build_deduplicated_csr(
        compact.N, compact.edge_dst, compact.edge_src, compact.edge_length
    )
    compact.find_nearest_node = lambda lat, lng: (
        (0 if lng < -87.6395 else 1),
        0.0,
    )
    compact.apply_risk_weights({"red": 0.95, "green": 0.10})
    _, csr_distance, _, csr_risk, _ = compact.compute_safe_route(
        41.8700, -87.6400, 41.8700, -87.6390
    )

    assert nx_distance == pytest.approx(260.0)
    assert csr_distance == pytest.approx(260.0)
    assert nx_risk == pytest.approx(0.10)
    assert csr_risk == pytest.approx(0.10)
