...
export function useStreetRisk(
  ...
  const lngKey = location ? location[0].toFixed(4) : undefined;
  const latKey = location ? location[1].toFixed(4) : undefined;

  useEffect(() => {
    if (lngKey === undefined || latKey === undefined || riskScore === null) {
