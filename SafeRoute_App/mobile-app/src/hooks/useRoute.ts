...
import type { LngLat, RouteProfile, RouteResponse } from "@/lib/types";
...
export function useRoute(
  ...
  useEffect(() => {
    ...
    setResult({ route: previousRoute, status: "loading" });
    ...
    (async () => {
      ...
      if (route) {
        ...
        setResult({ route, status: "ready" });
      } else {
        latestRouteRef.current = null;
        setResult({ route: null, status: "error" });
      }
    })();
  ...
  }, [startLng, startLat, endLng, endLat, profile, nonce]);
